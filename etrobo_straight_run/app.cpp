...
void main_task(intptr_t unused)
{
    ...
    // 周期ハンドラ開始
    sta_cyc(CYC_RIGHT_MOTOR);
    sta_cyc(CYC_LEFT_MOTOR);

    slp_tsk(); // バックボタンが押されるまで待つ

    // 周期ハンドラ停止
    stp_cyc(CYC_RIGHT_MOTOR);
    stp_cyc(CYC_LEFT_MOTOR);
 ...
 * 左モータ回転タスク
 */
void left_motor_task(intptr_t exinf)
{
    if (ev3_button_is_pressed(BACK_BUTTON))
    {
        wup_tsk(MAIN_TASK); // バックボタン押下
 ...
 * 右モータ回転タスク
 */
void right_motor_task(intptr_t exinf)
{
    if (ev3_button_is_pressed(BACK_BUTTON))
    {
        wup_tsk(MAIN_TASK); // バックボタン押下
