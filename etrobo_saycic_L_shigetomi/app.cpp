...
static void user_system_create()
{
    ...
    gLineTracer = new LineTracer(gScenarioTracer,
                                 gStarter,
                                 gWalker,
                                 gWalkerTimer);
...
void main_task(intptr_t unused)
{
    ...
    // 周期ハンドラ開始
    sta_cyc(CYC_TRACER);    // トレースタスク生成
    sta_cyc(CYC_DISTANCE);  // 距離測定タスクの生成
    sta_cyc(CYC_DIRECTION); // 距離測定タスクの生成

    slp_tsk(); // バックボタンが押されるまで待つ

    // 周期ハンドラ停止
    stp_cyc(CYC_TRACER);    // トレースタスクの終了
    stp_cyc(CYC_DISTANCE);  // 距離測定タスクの終了
    stp_cyc(CYC_DIRECTION); // 距離測定タスクの終了
 ...
 * 距離計測タスク
 */
void distance_task(intptr_t unused)
{
    // 距離測定update
 ...
 * 方位計算タスク
 */
void direction_task(intptr_t unused)
{
    // 方位計算update
